...
class Rest {
	...
	public function authorize() {
		...
		public function readUser($access_token, $user_id) {
			...
			$request = new HTTP_Request2($constants->HOSTNAME . $constants->USER_PATH . '/' . $user_id . '/' . 'courses', HTTP_Request2::METHOD_GET);
			$request->setHeader('Authorization', 'Bearer ' . $access_token);
                        $request->setConfig(array(
                                'ssl_verify_peer'   => $constants->ssl_verify_peer,
                                'ssl_verify_host'   => $constants->ssl_verify_host
		...
		public function readMembership($access_token, $course_id) {
			$constants = new Constants($this->clientURL);
			//$membership = new Membership();
		
			$request = new HTTP_Request2($constants->HOSTNAME . $constants->COURSE_PATH . '/' . $course_id . '/users',  HTTP_Request2::METHOD_GET);
